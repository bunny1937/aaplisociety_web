...
export async function notifyVisitorChange({
  ...
  await safe(async () => {
    ...
    if (memberId) {
      const data = { type, visitorId: String(visitorId), notificationId: String(notif._id), guardName: guardName || "" };
      // Only the OWNER of the flat is alerted about visitor activity. SOS is the
      // one exception — that is a safety event and goes to everyone on the flat.
      if (isSos) {
        await sendFcmToMember(String(memberId), { title: notif.title, body: notif.message }, data);
      } else {
        const owners = await User.find({ memberId, occupancyType: { $ne: "Tenant" } }).select("_id").lean();
        for (const o of owners) {
          await sendFcmToUser(String(o._id), { title: notif.title, body: notif.message }, data);
        }
      }
    }
  });
...
async function pushToFlat({ memberId, tenantsOnly }, payload, data) {
  const users = await User.find({
    memberId,
    occupancyType: tenantsOnly ? "Tenant" : { $ne: "Tenant" },
  })
    .select("_id")
    .lean();
