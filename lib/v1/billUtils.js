...
export function normalizeBill(b, member) {
  ...
  const memberBlock = {
    ...
    carpetAreaSqft: member?.carpetAreaSqft ?? plain.generationMetadata?.memberAreaAtGeneration ?? null,
    builtUpAreaSqft: member?.builtUpAreaSqft ?? null,
  };

  const advanceApplied = num(plain.advanceApplied);
  const directPaymentAmount = num(Math.max(0, amountPaid - advanceApplied));
  const displayStatus = balanceAmount > 0 && directPaymentAmount <= 0
    ? "Unpaid"
    : plain.status === "Partial" && directPaymentAmount > 0 ? "Paid (Partial)" : plain.status;
  return {
    ...plain,
    _id: String(plain._id),
    displayStatus,
    paymentAcknowledged: directPaymentAmount > 0,
    advanceApplied,
    directPaymentAmount,
    totalPayable: balanceAmount,
    ...
    areaSqft: memberBlock.carpetAreaSqft,
    builtUpAreaSqft: memberBlock.builtUpAreaSqft,
