...
async function sendToTokens(tokens, payload, data, label) {
  ...
  try {
    const resp = await messaging.sendEachForMulticast({
      ...
      data,
      // Rich, high-priority Android notification: brand color, big-picture
      // image when supplied, sound + vibration, MAX importance so it heads-up.
      android: {
        priority: "high",
        notification: {
          channelId: "high_importance_channel",
          color: payload.color || "#0C4E54",
          sound: "default",
