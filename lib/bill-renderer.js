...
export default function renderBillHtml(
  ...
  const previousBillHtml = null; // Reference copy removed — receipts tab shows payments instead
  ...
  // Ledger V2 (§4/§10): a renderer NEVER calculates interest. currInt/
  // monthInterest must come from the already-generated bill via
  // precomputedCurrInt/precomputedMonthInterest. The only remaining fallback
  // is for rendering an OLD pre-migration bill that never stored those
  // fields — never for a bill this session's GenerationService produced.
  let interestAmount = 0;
  let currInt = 0;
  let monthInterest = 0;
  const principalForInterest = data.prevRemPrincipal ?? data.previousBalance ?? 0;
  const remIntFromPrior = data.prevRemInt ?? 0;
  if (data.precomputedCurrInt != null || data.precomputedMonthInterest != null) {
    currInt = Number(data.precomputedCurrInt) || 0;
    monthInterest = data.precomputedMonthInterest != null ? Number(data.precomputedMonthInterest) : currInt;
    interestAmount = monthInterest;
  } else if (principalForInterest > 0 || remIntFromPrior > 0) {
  ...
  // Ledger V2: "Account is clear" must reflect the bill actually being
  // rendered, not just an unrelated historical unpaidBills query. Prefer the
  // persisted balanceAmount (§5); fall back to status for pre-migration bills
  // that never stored it.
  const currentBillBalance = Number(data.balanceAmount) || 0;
  const currentBillUnpaid =
    currentBillBalance > 0.005 ||
    ["Unpaid", "Partial", "Overdue"].includes(data.status);
  ...
  const chargeRows = Object.entries(breakdown)
    ...
    <tr style="background:${idx % 2 === 0 ? "#ffffff" : "#f9fafb"}">
      ...
      <td style="padding:10px 14px;border-bottom:1px solid #e5e7eb;font-size:14px;font-weight:500;color:#1f2937;">${escapeHtml(desc)}</td>
  ...
  const unpaidBillsHtml =
    ...
    <div style="margin-bottom:28px;">
        ...
        <tbody>
          ${(data.unpaidBills || [])
            ...
            <tr style="background:${i % 2 === 0 ? "#fff" : "#fff5f5"}">
              <td style="padding:8px 12px;border-bottom:1px solid #fecaca;">${escapeHtml(b.billPeriodId || b.period || "-")}</td>
              ...
              <td style="padding:8px 12px;border-bottom:1px solid #fecaca;text-align:center;">
                <span style="background:#fee2e2;color:#991b1b;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700;">${escapeHtml(b.status || "Unpaid")}</span>
              </td>
            </tr>`,
            )
            .join("")}
      ...
      : currentBillUnpaid
        ? `<div style="background:#fef3c7;border-radius:8px;padding:20px;text-align:center;color:#92400e;font-weight:600;margin-bottom:28px;">
          ⚠️ Current bill outstanding: ₹${currentBillBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
        </div>`
        ...
        <tbody>
          ${(data.recentTransactions || [])
            ...
            <tr style="background:${i % 2 === 0 ? "#ffffff" : "#eff6ff"}">
              ...
              <td style="padding:8px 12px;border-bottom:1px solid #bfdbfe;">${escapeHtml(tx.description || tx.category || "-")}</td>
              <td style="padding:8px 12px;border-bottom:1px solid #bfdbfe;text-align:center;">
                <span style="padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700;background:${tx.type === "Credit" ? "#d1fae5" : "#fee2e2"};color:${tx.type === "Credit" ? "#065f46" : "#991b1b"}">${escapeHtml(tx.type)}</span>
              </td>
...
<div style="max-width:800px;margin:0 auto;font-family:Arial,sans-serif;font-size:14px;color:#1f2937;">
  ...
  <div style="padding:40px;background:white;page-break-after:always;">
    ...
    <div style="background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;padding:28px 32px;border-radius:10px;margin-bottom:24px;">
      <h1 style="margin:0 0 6px 0;font-size:24px;font-weight:700;">${escapeHtml(society.name || "Society")}</h1>
      <p style="margin:0;font-size:12px;opacity:0.85;">${escapeHtml(society.address || "")}</p>
    </div>
    <!-- Bill Title Row -->
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid #e5e7eb;">
      <h2 style="margin:0;font-size:20px;font-weight:700;letter-spacing:1px;color:#1e40af;">${data.billSeries === "COMMERCIAL" ? "COMMERCIAL UNIT BILL" : "MAINTENANCE BILL"}</h2>
      <div style="text-align:right;font-size:12px;color:#6b7280;line-height:1.8;">
        <div>Bill No: <strong style="color:#1f2937;">${data.billSeries === "COMMERCIAL" && data.billNo ? escapeHtml(data.billNo) : `${escapeHtml(data.billPeriod)}-${escapeHtml(member.flatNo || "")}`}</strong></div>
    ...
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:18px 22px;display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;font-size:13px;">
      <div><span style="color:#6b7280;">Bill Period:</span> <strong>${escapeHtml(data.billPeriod)}</strong></div>
      <div><span style="color:#6b7280;">Flat:</span> <strong>${escapeHtml(member.wing || "")}-${escapeHtml(member.flatNo || "")}</strong></div>
      <div><span style="color:#6b7280;">Owner Name:</span> <strong>${escapeHtml(member.ownerName || "")}</strong></div>
      <div><span style="color:#6b7280;">Area billed on:</span> <strong>${areaBasisLabel(member, { areaBasis: config.areaBasis })}</strong></div>
      <div><span style="color:#6b7280;">Contact:</span> <strong>${escapeHtml(member.contactNumber || "-")}</strong></div>
      ...
      <div style="background:rgba(255,255,255,0.5);border-radius:6px;padding:12px 16px;font-size:12px;color:#7f1d1d;line-height:1.7;">
        ...
        ${
          currInt > 0
            ...
            : (remIntFromPrior > 0 ? ` = ₹0 new (carried: ₹${remIntFromPrior.toLocaleString("en-IN", { minimumFractionDigits: 2 })})` : " = ₹0 (no outstanding principal)")
        }
        <br/>Simple interest on unpaid principal only — never on interest. See Page 2 for full account statement.
      </div>
    </div>`
        : currentBillUnpaid
          ? `
    <div style="background:#fef3c7;border:1px solid #fde68a;border-left:5px solid #f59e0b;padding:14px 20px;border-radius:8px;margin-bottom:20px;font-size:13px;color:#92400e;font-weight:600;">
      No previous outstanding balance — this month's charges are still due.
    </div>`
          : `
    ...
    <div style="border:1px solid #e5e7eb;border-radius:8px;padding:18px 22px;margin-bottom:20px;font-size:12px;color:#6b7280;">
      ...
      <ol style="margin:0;padding-left:18px;line-height:2;">
        <li>Please pay on or before <strong style="color:#dc2626;">${formatDate(dueDate)}</strong> — any unpaid principal remaining at next month's bill generation will accrue further interest.</li>
        <li>Interest @ ${interestRate}% p.a. (${interestMethod}, simple, non-compounding) is charged on unpaid principal only.</li>
  ...
  <div style="padding:40px;background:white;border-top:4px solid #1e40af;margin-top:4px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;padding-bottom:16px;border-bottom:2px solid #e5e7eb;">
      <div>
        ...
        <p style="margin:0;font-size:13px;color:#6b7280;">${escapeHtml(society.name || "")} — ${escapeHtml(member.wing || "")}-${escapeHtml(member.flatNo || "")} — ${escapeHtml(member.ownerName || "")}</p>
      </div>
      <div style="text-align:right;font-size:12px;color:#6b7280;line-height:1.8;">
        <div>Period: <strong>${escapeHtml(data.billPeriod)}</strong></div>
        <div>Flat: <strong>${escapeHtml(member.wing || "")}-${escapeHtml(member.flatNo || "")}</strong></div>
...
<td style="padding:7px 12px;font-weight:600;">Monthly, simple interest on unpaid principal only — no grace period, no compounding</td></tr>
...
<td style="padding:9px 12px;font-weight:700;font-size:15px;color:#92400e;">₹${interestAmount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td></tr>
    ...
    <div style="text-align:center;font-size:11px;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:16px;margin-top:28px;">
      ${escapeHtml(society.name || "")} &nbsp;|&nbsp; ${escapeHtml(society.address || "")} &nbsp;|&nbsp; Page 2 of 2 &nbsp;|&nbsp; Computer Generated Bill
    </div>
  </div>
</div>`;
