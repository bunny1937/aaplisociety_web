 ...
 */
export async function process(event, { session: externalSession } = {}) {
  ...
  const runner = (session) => processWithinSession(event, session);
...
async function processWithinSession(event, session) {
  ...
  // 7. Flip the voucher Draft → Posted, linking its journal entry.
  await markVoucherPosted(voucher._id, journalEntry._id, session);
