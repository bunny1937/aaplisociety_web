...
export async function persistJournalEntry({
  ...
  // `ordered: true` is required by mongoose/driver when passing a session
  // together with an array of >1 documents to .create() — omitting it throws
  // "Cannot call `create()` with a session and multiple documents unless
  // `ordered: true` is set" for every journal entry with 2+ lines (i.e. all
  // of them). Found by the accounting-smoke.unit.test.js end-to-end test.
  await JournalLine.create(lineDocs, { session, ordered: true });
