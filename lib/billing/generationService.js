...
/** Single carry-forward lookup (§2/§14) — replaces the old N-bill scan. */
export async function resolveOpeningBalances({ memberId, societyId, year, month, member }, sessionOpts = {}) {
  const prevBill = await Bill.findOne({
    memberId,
    societyId,
    billPeriodId: prevPeriodId(year, month),
    isDeleted: { $ne: true },
  })
    .select("billPeriodId closingPrincipal closingInterest billPrincipalBalance billInterestBalance principalBalance interestBalance")
  ...
  if (prevBill) {
    return {
      prevBill,
      openingPrincipal: twoDp(prevBill.closingPrincipal ?? prevBill.billPrincipalBalance ?? prevBill.principalBalance ?? 0),
      openingInterest: twoDp(prevBill.closingInterest ?? prevBill.billInterestBalance ?? prevBill.interestBalance ?? 0),
    };
  }
...
async function persistBill({
  ...
  const pushDate = safeConfigDate(year, month, society?.config?.billPushDay || 1);
 ...
 */
export async function generateBill({ societyId, memberId, year, month, performedBy }) {
  ...
  return persistBill({ societyId, memberId, year, month, performedBy, ...ctx, openingPrincipal, openingInterest });
}
