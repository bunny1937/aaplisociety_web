...
const exactDue = society?.config?.billDueDate
  ? new Date(society.config.billDueDate)
  : null;

const exactValid =
  exactDue && !Number.isNaN(exactDue.getTime());

if (!exactValid) {
  const error = new Error(
    "Bill Due Date is not configured for this society.",
  );

  error.code = "BILL_DUE_DATE_NOT_CONFIGURED";
  throw error;
}

const exactPeriod =
  `${exactDue.getFullYear()}-${String(
    exactDue.getMonth() + 1,
  ).padStart(2, "0")}`;

/*
 * For the configured/import period, use the exact selected date.
 * For later billing periods, reuse the selected day of the month.
 *
 * Never calculate the due date using:
 * generation date + interestAfterDays.
 */
const configuredDue =
  billPeriodId <= exactPeriod
    ? new Date(
        exactDue.getFullYear(),
        exactDue.getMonth(),
        exactDue.getDate(),
        0,
        0,
