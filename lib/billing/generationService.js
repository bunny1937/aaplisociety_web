...
export async function generateBillsForMembers({ societyId, memberIds, year, month, performedBy, publishMode = "config", scheduledFor = null }) {
  const results = { generated: [], failed: [] };
  for (const memberId of memberIds) {
    try {
      const bill = await generateBill({ societyId, memberId, year, month, performedBy, publishMode, scheduledFor });
