...
const DEFAULTS = {
  ...
  defaultAccountMappings: {
    ...
    // Liability credited when a member pays more than their outstanding dues.
    memberAdvanceAccountId: null,
  },
  ...
  healthScoreWeights: {
    ...
    draftVouchers: 10,
    depreciation: 15,
    bankReconciliation: 15,
    scheduleCoverage: 10,
    otherValidations: 5,
  },
};

// True for a Mongoose/BSON ObjectId instance (duck-typed, not instanceof —
// deliberately, since a value can come from a different bundled
// mongoose/bson module instance than the one this file imports, e.g. under
// Next.js dev's per-route webpack bundling, where instanceof silently fails).
...
function deepMerge(defaults, stored) {
  ...
  for (const key of Object.keys(defaults)) {
    ...
    if (looksLikeObjectId(storedVal)) {
      // Store as a plain hex string, never the ObjectId instance itself —
      // Mongoose casts a string to ObjectId unconditionally on save, which
      // sidesteps any cross-module-instance identity mismatch entirely.
      out[key] = storedVal.toHexString();
    } else if (
      defaultVal &&
