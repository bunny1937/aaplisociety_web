...
export class FlexiblePDFGenerator {
  // Accepts either a filesystem path (legacy) OR a Buffer / { buffer } object
  // (new: templates now live in MongoDB since Vercel's FS is read-only).
  // `customPositions` (optional) lets a per-society uploaded template override
  // the default overlay coordinates in PDF_FIELD_POSITIONS — set by the admin
  // in the bill-template designer's PDF field-position editor and stored on
  // society.billTemplate.pdfFields. Used only for PDFs with no fillable form
  // fields (the overlay/draw path below); form-field PDFs are filled by name.
  constructor(template, customPositions) {
    if (Buffer.isBuffer(template)) {
      this.templateBuffer = template;
    } else if (template && Buffer.isBuffer(template.buffer)) {
      this.templateBuffer = template.buffer;
    } else {
      this.templatePath = template;
    }
    this.positions = customPositions && Object.keys(customPositions).length
      ? { ...PDF_FIELD_POSITIONS, ...customPositions }
      : PDF_FIELD_POSITIONS;
  }
  async generateBill(billData) {
    try {
      // Read the template from the in-memory buffer when available, else fall
      // back to reading the legacy on-disk path.
      const templateBuffer =
        this.templateBuffer || fs.readFileSync(this.templatePath);
  ...
  // Generic overlay filler: given an arbitrary flat { fieldKey: value } object
  // (bill overlay keys like companyName/invoiceNumber/... OR receipt keys
  // like receiptNumber/payerName/... — whatever this.positions was built
  // with), fills PDF form fields by name when the template has them, else
  // draws each field at its configured x/y (this.positions[fieldKey]).
  // Used by the uploaded-image bill flow and by ALL receipt generation
  // (uploaded-pdf + uploaded-image), which have their own field vocabulary
  // and can't reuse the bill-specific generateBill() above.
  async generateGenericOverlay(fieldsData, formFieldData) {
    const templateBuffer =
      this.templateBuffer ||
      (this.templatePath ? fs.readFileSync(this.templatePath) : null);
    if (!templateBuffer) {
      throw new Error("generateGenericOverlay requires a PDF template");
    }
    const pdfDoc = await PDFLibDocument.load(templateBuffer);
    const form = pdfDoc.getForm();
    const fields = form.getFields();
    if (fields.length > 0 && formFieldData) {
      this._fillFormFields(form, formFieldData);
      form.flatten();
      return await pdfDoc.save();
    }
    ...
    Object.entries(fieldsData || {}).forEach(([key, value]) => {
      this._addTextField(page, key, value, fontRegular, fontBold);
    });
    return await pdfDoc.save();
  }
  // Composites an uploaded image (JPG/PNG) as a full-page background on a
  // brand-new single-page PDF, then draws the configured overlay fields on
  // top — same _addTextField/this.positions mechanism as the PDF overlay
  // path above, so image templates use the exact same field-position config
  // shape the admin UI already edits for PDFs.
  async generateImageOverlay(imageBuffer, mimeType, fieldsData) {
  ...
  _addTextField(page, fieldKey, value, fontRegular, fontBold) {
    const config = this.positions[fieldKey];
  ...
  _addTable(page, items, fontRegular, fontBold) {
    ...
    const startPos = this.positions.tableStart;
